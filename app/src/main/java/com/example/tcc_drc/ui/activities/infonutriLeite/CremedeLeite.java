        ...
        if (cur != null) {
            ...
            if (cur.getInt(0) == 0) {
                try {
                    ...
                    stmt.bindString(1, "Informação Nutricional: Creme de Leite");
                    stmt.executeInsert();

                    stmt.bindString(1, "Porção: 1 1/2 colher de sopa 15g ");
                    stmt.executeInsert();

                    stmt.bindString(1, "Valor energético (Kcal): 33,2");
                    stmt.executeInsert();
                    ...
                    stmt.bindString(1, "Sódio (mg): 7,7");
                    stmt.executeInsert();

                    stmt.bindString(1, "Fósforo (mg): 17,8");
                    stmt.executeInsert();

                    stmt.bindString(1, "Potássio (mg); 17,8");
                    stmt.executeInsert();
                    bancoDados.close();
                    listarDados();
